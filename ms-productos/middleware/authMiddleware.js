...
export const mismoUsuarioOStaff = (param = 'usuarioId') => {
  return (req, res, next) => {
    const rolesStaff = ['administrador', 'empleado']

    if (rolesStaff.includes(req.user?.role) || req.user?.id === req.params[param]) {
      return next()
    }

    res.status(403).json({ msg: 'Ese recurso no es tuyo' })
  }
}
